...
await test('AI request selects current model and explicit non-thinking JSON mode',()=>{const p=HWT.aiPayload('JSON',{});assert.match(HWT.aiPayload('修正元数据',{}).messages[0].content,/JSON/);assert.equal(p.model,'deepseek-flash');assert.equal(p.thinking.type,'disabled');assert.equal(p.response_format.type,'json_object');assert.equal(HWT.aiPayload('text',{},false).response_format,undefined);});
