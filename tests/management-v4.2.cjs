...
let failSave=false,calls=[],pending;const data={lesson,attempts:[structuredClone(attempt)],rosters:[],truncated:false};
...
w.module.exports.mount({host:w.document.querySelector('#host'),ask:async()=>'',run:fn=>{pending=fn();return pending;},improve:async()=>{}});
